...
def prepare_batch(
    ...
    for idx in range(boxes.shape[0]):
        ...
        if emotion_labels is not None:
            # 假设 emotion_labels 是一个 list 或 array，长度与 boxes 相同
            # 如果某个框没有标签（例如背景人物），可以设为 -100
            label = emotion_labels[idx]
            data_info["emotion_label"] = torch.tensor(label, dtype=torch.long)
        else:
            # 如果完全没有提供标签，给一个默认忽略值 (通常 CrossEntropyLoss 默认 ignore_index=-100)
            data_info["emotion_label"] = torch.tensor(-100, dtype=torch.long)
